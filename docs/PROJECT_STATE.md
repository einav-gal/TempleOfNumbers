...
### Crystal-return mechanism — Stage 1 infrastructure (completed)

Goal: once all three crystals are collected and the player returns to the
Central Hall, show a system letting the player drag/click the collected
crystals back into fixed slots near the Heart of the Temple. This task was
explicitly scoped to infrastructure only — placement, persistence, and
placeholder feedback — not the eventual ring-opening/ending sequence.

**State**: reused the existing registry-backed system in `GameState.ts`
rather than inventing a parallel tracker. Added one new slice,
...
`setCrystalPlaced`/`areAllCrystalsPlaced`, plus `areAllCrystalsCollected`
(a small missing helper over the *existing* `CrystalCollectionState`) and
an exported canonical `CRYSTAL_IDS` order (`['pink','red','green']`) for
new code to read instead of re-declaring its own copy (`CrystalHolder.ts`
predates this and keeps its own local `CRYSTAL_ORDER` — left untouched,
still correct, just not the one new code reads from now).

**New `src/game/CrystalPlacementMode.ts`**: only constructed by
`CentralHallScene` when `areAllCrystalsCollected(registry)` is true — if
any crystal is still missing, nothing is created and the hall behaves
exactly as before. When active:
- **`createCollectedCrystalsTray()`**: one small procedural gem per
  crystal (tinted per crystal, same palette `CrystalHolder.ts` uses),
  anchored near the bottom of the hall.
- **`createCrystalSlots()`**: one fixed slot per crystal — **not** a
  hardcoded count of 4, but generated by mapping over the same
  `CRYSTAL_IDS` list the rest of the collection system already uses (so
  the slot count always tracks the real number of crystals in the game).
  Placeholder art only (no dedicated slot asset exists yet — see the note
  below on what to commission): a gentle dashed-diamond recess plus a
  soft tinted glow, both procedurally generated, positioned in a row
  below the pedestal (near the rings mechanism, without covering it).
- **Which crystal fits which slot**: a fixed 1:1 mapping by crystal id
  (`SLOT_ID_BY_CRYSTAL` — `pink → crystalSlot1`, `red → crystalSlot2`,
  `green → crystalSlot3`, the same left-to-right order `CrystalHolder.ts`
  already uses for its pouch). There is no puzzle about *which* slot a
  crystal belongs in — each has exactly one correct destination — only
  about actually placing it there.
- **Drag**: crystals are real Phaser Containers made draggable via
  `scene.input.setDraggable()`; `handleCrystalDrop()` measures the
  dropped crystal's distance to every slot with a generous catch radius
  bigger than the slot's own visual size, and calls `placeCrystalInSlot()`
  only if the nearest slot within that radius is both the crystal's own
  correct slot and not already filled — otherwise `returnCrystalToStart()`
  tweens it smoothly back to its tray position. Both the tray and the
  slots are anchored in background-image-pixel space (the same
  convention every other Central Hall object — Pot, Handle, Statue,
  Doorway — already uses, deliberately **not** `scrollFactor(0)`), so a
  dragged crystal and its target slot share one coordinate system and
  Phaser's own camera-aware drag system (`dragX`/`dragY`, already derived
  from `pointer.worldX`/`worldY` — confirmed in an earlier sprint) keeps
  working correctly at any pinch-zoom/pan state, with no hand-rolled
  screen-coordinate math of the kind that caused the Pink Room ring bug.
- **Click-to-place** (mobile-friendly alternative to dragging):
  `selectCrystal()`/`onSlotClicked()` — clicking a crystal gives it a
  soft selection glow; clicking its own correct, empty slot afterward
  places it the same way a drag would; clicking a wrong slot is an
  explicit no-op (the crystal stays selected).
- **Locking**: `placeCrystalInSlot()` marks both the crystal and slot as
  filled, disables the crystal's own interactivity
  (`disableInteractive()`), plays a brief glow flash + small scale-pop
  ("locked in"), then calls `onCrystalPlaced(crystalId, slotId)` — which
  saves progress via `setCrystalPlaced()` and, once
  `areAllCrystalsPlaced()` is true, calls `onAllCrystalsPlaced()`, which
  for now only shows a temporary centered message ("כל הגבישים הוחזרו
  למקומם") — the full ring-opening/ending sequence is explicitly the next
  stage, not built here.
- **Persistence**: `createCollectedCrystalsTray()` reads
  `getCrystalPlacementState()` on every `create()` — a crystal already
  placed on a previous visit never gets a tray gem at all; instead its
  slot is immediately shown filled (glow up, no animation replay), and
  `layout()`'s per-crystal branch keeps an already-placed crystal
  positioned on its slot through any resize. `onAllCrystalsPlaced()` only
  ever fires from a placement made *this* session, never from the
  restore path, so returning to an already-fully-placed hall doesn't
  re-show the message.
- **Mobile/ENVELOP awareness**: both the slot row and the tray row shift
  to a tighter, higher Y position in `MobilePinchZoom`'s cropped ENVELOP
  scale mode (same `isEnvelopScaleMode()` pattern `CrystalHolder.ts`/
  `Room3Scene.ts`/`IntroOverlay.ts` already use), so they stay inside the
  visible band on short phone-landscape screens instead of being cropped
  off the bottom edge.

**`CentralHallScene.ts` changes**: one new field
(`crystalPlacementMode`), conditional construction right after
`crystalHolder.create()`, a `layout()` call passing the scene's own
`toScreenX`/`toScreenY`/`backgroundScale`, and `destroy()` in the existing
`SHUTDOWN` handler alongside every other per-scene helper. No other hall
object's behavior, position, or interactivity was touched.

**Placeholder assets — recommended for a future art pass**: no dedicated
graphics exist yet for either the mechanism slots or the crystal icons
used outside `CrystalHolder`'s own small pouch gem; both are procedurally
drawn (a dashed-diamond recess for slots, a faceted-gem shape for the
tray/held crystals). Worth commissioning later: three (or more, if the
crystal count ever grows) carved-stone slot recesses matching the
pedestal's material/style, and higher-fidelity crystal icons matching each
room's actual crystal asset (`OBJ_001-crystal.png` for pink,
`green-crystal.png` for green, etc.) rather than the current neutral
tinted-gem placeholder.

Files changed: `GameState.ts` (new placement-state slice + helpers +
exported `CRYSTAL_IDS`), `CentralHallScene.ts` (wiring only). File
created: `CrystalPlacementMode.ts`. No other scene, puzzle, or existing
Central Hall interaction was touched.

`npm run build` (`tsc && vite build`) passes with no errors.

### Crystal-return mechanism — clarity pass, richer glow, and a debug shortcut (completed)

Feedback on stage 1: the mechanism worked but gave the player no idea what
to do, the slots/gems were placeholder-plain, and there was no fast way to
test the stage without replaying all three rooms. This pass adds: an
instruction line, clearly-shaped/positioned diamond slots with proximity
feedback, richer "magical" crystal glow with hover/drag/lock states, a
small ring-nudge on successful placement, dimming the now-redundant corner
pouch, and an isolated `?debug=final` shortcut. Still no full
ring-opening/ending sequence — unchanged from the previous stage's scope.

**Guidance + slot layout (`CrystalPlacementMode.ts`):**
- A short Hebrew instruction ("גררו כל גביש אל השקע המתאים במנגנון")
  anchored above the slot row.
- Slots repositioned from a plain horizontal row into pink-left/red-top/
  green-right around a fixed mechanism-center anchor (mirroring, not
  reaching into, `HeartOfTheTemple`'s own crystal/rings center) — per-mode
  (FIT/ENVELOP) offset tables, same `isEnvelopScaleMode()` pattern as
  before, so the top slot and the guidance text both stay clear of
  ENVELOP's ~190bg-px top crop.
- Slots redrawn to read as engraved/sunken into metal (a gradient inset
  fill + a bright highlight along the upper-left "lip" edge) rather than a
  flat dashed outline, sized a bit larger than the gems themselves, and
  now visibly glowing (a modest "rest" alpha, not zero) even before any
  drag starts, so they're legible up front — matching the "clear enough
  before dragging" requirement.
- Slot glow now has four animated states (`SlotVisualState`: rest →
  targeted → near → filled) instead of a flat filled/unfilled toggle: the
  crystal's own matching slot highlights the instant a drag starts, and
  intensifies further once within the catch radius; `layout()` re-applies
  whichever state is current (`applySlotVisualStateInstant()`) rather than
  a flat default, so a resize mid-interaction stays visually consistent.

**Richer crystal visuals (`CrystalPlacementMode.ts`):** each tray crystal
now has its own dedicated glow layer (a separate, non-interactive `Image`
behind the gem, added first so it never covers the gem's own facet detail
or its hit area) with a two-tone tint distinct from the gem's own color
(`CRYSTAL_GLOW_TINTS` — pink-violet, red-orange, green-turquoise) and:
- A continuous, slow "breathing" idle animation (gentle alpha + scale
  oscillation, staggered per crystal so the three don't pulse in lockstep)
  — the ONLY tween that ever writes to the glow's alpha/scale; hover,
  click-selection, and dragging each apply an additional boost that's
  recombined with the current breathing value in one place
  (`applyCrystalGlowVisual()`), so nothing ever fights over the same
  animated property.
- Hover/touch: gem scales up slightly, glow strengthens, `pointer` cursor
  (`container.input.cursor`).
- Dragging: elevated depth (unchanged from before), a stronger glow boost,
  and a few soft sparkle particles trailing the crystal (one shared,
  reusable `ParticleEmitter`, repositioned/retinted per drag rather than
  one emitter per crystal).
- Locking: a brief bright flash, then the glow settles to a steady,
  non-breathing alpha (the breathing tween is explicitly stopped) —
  distinguishing "already handled" from "still needs attention."

**"Mechanism activated" feedback:** `HeartOfTheTemple.ts` gained
`playActivationNudge()` — a small, self-decaying extra rotation applied
only to the middle ring, added on top of (never replacing) its existing
continuous oscillation math (which is recomputed fresh every frame from a
fixed base angle — the nudge is passed in as an extra additive term rather
than fighting that recomputation). `CentralHallScene.ts` wires
`crystalPlacementMode.onCrystalPlaced = () => this.heart?.playActivationNudge()`
— a new public callback property on `CrystalPlacementMode`, separate from
its own internal (private) `onCrystalPlaced`-equivalent bookkeeping method,
so external scenes can react to a placement without reaching into the
class's internals.

**De-duplicating the crystal display:** `CrystalHolder.ts` gained
`setDimmed(dimmed: boolean)` (just an alpha change — the pouch was never
actually clickable, so nothing about its interactivity changes).
`CentralHallScene.ts` calls it once, right when `CrystalPlacementMode` is
constructed, so the corner pouch and the new bottom tray never both show
the same collected crystals at full strength; the tray is the only place
a crystal can actually be dragged from.

**Debug mode (`?debug=final` / `?debug=reset`):** see the dedicated
"Debug mode" entry in the Current State Summary below for the full
description and the exact URLs. Implemented as new exported functions in
`GameState.ts` (`isDebugFinalStageRequested`/`isDebugResetRequested`/
`isDebugModeActive`/`applyDebugFinalStage`/`clearDebugState`), read once in
`main.ts` synchronously right after the `Phaser.Game` is constructed (safe
because Phaser's own scene boot happens asynchronously, on the next tick —
always after this point, never before), plus a small always-present,
pointer-events-none HTML tag (`#debug-stage-tag` in `index.html`) shown
only via `isDebugModeActive()`.

Files changed: `CrystalPlacementMode.ts` (rewritten in place — same public
`create()`/`layout()`/`destroy()` contract, same underlying drag/click/
persistence logic from the previous stage), `HeartOfTheTemple.ts`
(`playActivationNudge()`), `CrystalHolder.ts` (`setDimmed()`),
`CentralHallScene.ts` (wiring only), `GameState.ts` (debug-mode functions),
`main.ts` (applies debug state at boot, sets up the debug tag),
`index.html` (debug tag markup + CSS). No puzzle logic, object position
elsewhere in the hall, or room-transition code was touched.

`npm run build` (`tsc && vite build`) passes with no errors.

### Heart of the Temple — real ring-opening mechanism (completed)

Feedback: the previous "activation nudge" was only ever a rotation, never
a real caged->open transition, and the rings needed to be confirmed as
genuinely separate objects before a real opening could be built at all.

**Pre-implementation check (reported before writing code, per this task's
own request):** `HeartOfTheTemple.ts` already has 3 separate logical
rings (`ringBack`/`ringMiddle`/`ringFront`), each a rear+front PNG *pair*
— 6 total `Phaser.GameObjects.Image` instances, all independent sprites
under `assets/images/central-hall/Rings/` (never baked into the hall
background), each already independently positionable/rotatable/scalable.
No new art was needed for a real opening — only new code — since each
pair can already move as one unit to any position/angle/scale.

**`HeartOfTheTemple.ts`** (substantially restructured — see the full
description in the "Central Hall" entry of the Current State Summary
below): the 3 separate ring objects were generalized into a `RingRuntime`
array (each carrying its own layout config, oscillation config, current
angle, and an `openProgress` 0-1 value), a `caged -> opening -> open`
state machine, and a real multi-phase opening sequence (shake -> align ->
each ring tweens its OWN position/angle/scale to its own open target,
staggered -> stronger sustained glow) built from genuine per-ring
Phaser tweens — not a single shared rotation. `layout()` now re-derives
every ring's position from its current `openProgress`/`angleDeg` (whatever
phase the mechanism is currently in) instead of always resetting to the
closed layout, so a resize mid-sequence (or after opening) stays correct.
`playActivationNudge(crystalId)` now nudges only the ONE ring tied to that
crystal, and checks `GameState.areAllCrystalsPlaced()` itself (the same
shared state `CrystalPlacementMode` already writes — no separate counter)
to decide whether this was the last one and the full sequence should
start. New `isOpen()`/`restoreOpen()` (the latter for the "restore, don't
replay" case — a returning visit where the mechanism was already opened).

**`CentralHallScene.ts`:** `crystalPlacementMode.onCrystalPlaced` now
passes the `crystalId` through to `heart.playActivationNudge(crystalId)`;
calls `heart.restoreOpen()` right after `heart.create()` if
`areAllCrystalsPlaced()` is already true (before the first `layout()`,
matching every other "restore" pattern in this file); and the crystal's
click handler now branches on `heart.isOpen()` — still true while caged
(the existing English "dormant" popup, `openPopup()`, untouched), a new
`openFinalStagePopup()` (same popup shell, its own Hebrew RTL message)
once open. The real ending/next-stage sequence itself is intentionally
NOT built here — this popup only confirms the click is wired correctly.

**`CrystalPlacementMode.ts`:** removed the now-redundant "all placed"
toast message (`showTemporaryMessage`/`ALL_PLACED_MESSAGE` and their
supporting fields) — the real ring-opening sequence, triggered from the
exact same `onCrystalPlaced` hook, is now that feedback, and showing both
at once would just be clutter on top of it.

Files changed: `HeartOfTheTemple.ts` (major rewrite), `CentralHallScene.ts`
(wiring + `openFinalStagePopup()`), `CrystalPlacementMode.ts` (removed the
superseded toast). No puzzle logic, other room, or unrelated Central Hall
...
- **Hint system (`HintSystem.ts`):** a small, screen-fixed "רמז" (hint)
  ...
  *available* (e.g. the floor tile only counts once the Pink Room shard is
  ...
  via the scene-wide dismiss listener. Confirmed still too small on a
  real device (button + crystal pouch), so both now scale up as a whole
  specifically in ENVELOP mode: `HintSystem.ts`'s button container gets
  `ENVELOP_BUTTON_SCALE` (1.3), `CrystalHolder.ts`'s own container gets
  its exported `ENVELOP_HOLDER_SCALE` (1.4) — both growing down-right
  from their fixed top-left anchor, never re-baked into the generated
  textures. `layout()` computes the button's Y position from the
  holder's *effective* (scaled) height, not the raw `HOLDER_HEIGHT_PX`
  constant, so the two stay correctly grouped in both modes. Fixing this
  exposed a real, previously latent bug in
  `CrystalHolder.getSlotScreenPosition()`: it added the slot's local
  offset to the container's position with no scale factor, which
  happened to work only because the container was always at scale 1
  before now — reward-crystal flights
  ...
  `CrystalHolder.ts` exports `ENVELOP_HOLDER_MARGIN_X_PX` (40, vs.
  `HOLDER_MARGIN_X_PX`'s 18) as its own X anchor in that mode;
  `HintSystem.ts` imports and reuses the same constant for its button, so
  the two stay aligned. A further real bug surfaced when the pouch was
  then reported invisible in *every* room: `CrystalHolder.ts` only ever
  applied its position/scale once, in `create()` — if the Scale
  Manager's mode changed afterward (a real, documented risk on mobile
  browsers, whose reported viewport shape can be unreliable right at page
  load), the pouch stayed frozen at whatever the first, possibly-wrong
  mode computed, while `HintSystem.ts`'s own `layout()` (already
  re-invoked on every resize) self-corrected. `CrystalHolder.ts` now has
  its own `layout()` doing the same position/scale work, called from
  every owning scene's own resize-triggered `layout()` (`CentralHallScene`
  /`PinkRoomScene`/`LibraRoomScene`/`Room3Scene`), the same pattern
  `HintSystem.ts` already used. Reads the shared registry via
...
- **Crystal-return mechanism (`CrystalPlacementMode.ts`):** once all three
  ...
  While this mode is active, `CrystalHolder`'s own corner pouch is dimmed
  (`CrystalHolder.setDimmed(true)`) so the same collected crystals aren't
  shown at full strength twice — the tray at the bottom is the only place
  a crystal can actually be dragged from. Progress (`crystalPlacement`
  registry slice) persists across visits. Placing the LAST crystal now
  triggers the real ring-opening sequence directly (no separate "all
  placed" toast anymore — the opening sequence itself is that feedback).
- **Heart of the Temple — ring-opening mechanism (`HeartOfTheTemple.ts`):**
  a small state machine, `caged` (default) -> `opening` (one-time,
  automatic) -> `open`:
  ...
  - Each of the 3 rings is tied to exactly one crystal (matching
    ...
    (`playActivationNudge(crystalId)` -> `playSingleRingOpen()`), never
    waiting for the other two: green -> the ring that falls to the LEFT,
    pink -> the ring that falls to the RIGHT, red -> the ring that falls
    straight down. Every ring's fall is a real shake -> align -> fall
    sequence (its own position/angle/scale tween, `Bounce.Out` easing so
    it visibly lands) — a genuinely separate, self-contained animation
    per ring, not a shared batch. A ring that has already fallen
    (`RingRuntime.hasOpened`) is permanently excluded from the idle sway
    in `update()` from that point on, independent of the other two (which
    keep swaying gently until their own crystal goes in).
  - Once `GameState.areAllCrystalsPlaced()` is true — checked fresh inside
    `playActivationNudge()` itself right as that (the last) crystal's ring
    starts falling, no separate counter to get out of sync — that ring's
    own fall is chained into the finale once it lands: the crystal's glow
    briefly builds up, then **the crystal itself shatters** — it vanishes
    (a quick scale/alpha collapse) while a one-shot burst of **1000**
    procedural glowing shard particles explodes outward from its
    position, fast/far enough to spread across the whole screen
    (`playCrystalExplosion()` — a dedicated small shard texture, not
    copies of the crystal's own photo-real image, which would look like
    many tiny whole crystals rather than fragments). The crystal is not
    interactive during or after this phase — there is nothing left to
    click.
  - A beat after the shatter, `HeartOfTheTemple.onMechanismShattered`
    fires automatically (never from a click) — wired in
    `CentralHallScene.ts` to `openFinalStagePopup()`, the game's actual
    "you won" message ("הצלחתם לצאת מהמקדש!"), using the same popup shell
    as the existing "dormant crystal" popup. Nothing further (credits, a
    dedicated ending scene, restart) is built beyond this message yet — a
    separate future task.
  - **`open`:** the mechanism's final state — every ring has fallen,
    crystal gone, the "you won" message shown. `HeartOfTheTemple.isOpen()`
    still exists for `CentralHallScene` to check if needed later, but the
    crystal itself can no longer be clicked at all once shattered.
  - **Rings disappear once landed:** each ring fades out and
    `setVisible(false)`s itself a short beat after its own fall/bounce
    completes (`fadeRingAway()`) — resolves the earlier known limitation
    about the falling ring's drop distance possibly exiting the visible
    band on the cropped mobile ENVELOP scale mode, since exactly where it
    lands no longer matters once it's gone again moments later.
  - **Restore, don't replay:** if `areAllCrystalsPlaced()` is already true
    when `CentralHallScene.create()` runs (a returning visit),
    `HeartOfTheTemple.restoreOpen()` jumps straight to the fully-open,
    already-shattered state before the first `layout()` call (rings
    already faded/hidden, crystal hidden) — no shake/align/open/explosion/
    fade replay, same pattern as `Statue.restoreOpen()`/
    `Entrance.restoreRevealed()`.
- **Debug shortcut to this stage** (testing only, see "Debug mode" below):
  visiting the game with `?debug=final` in the URL jumps straight here
  with all three crystals collected but none yet placed — placing all
  three manually now plays the real ring-opening sequence through to the
  end.
...
- **postFX Glow skipped on real mobile devices (`isMobileDevice()`,
  `scaleMode.ts`):** reported repeatedly from a real phone — every
  crystal/pedestal using `image.postFX?.addGlow(...)` (`PinkCrystal.ts`,
  `HeartOfTheTemple.ts`, `Room3Scene.ts`'s pedestal + big crystal,
  `MapFractionPuzzle.ts`'s solved-map glow) rendered as a large,
  broken-looking horizontal light streak shooting sideways off-screen,
  in that crystal's own tint color, instead of a soft halo. Originally
  gated on `isEnvelopScaleMode()` (a working theory that the WebGL Glow
  FX pipeline's render-target resolution mismatches the real canvas
  specifically in ENVELOP mode), but the bug kept recurring on real-
  device reports even after that shipped — the reporting device's
  viewport apparently wasn't narrow/short enough to actually trigger
  ENVELOP at the time, so that gate silently never applied there. Now
  gated on `isMobileDevice()` instead (checks `scene.sys.game.device.os`
  for Android/iOS/Windows Phone — a property of the actual device, not
  the current viewport shape/scale mode), which more reliably points at
  the real cause: a mobile-GPU/WebGL compatibility issue with Phaser's
  Glow FX pipeline itself. `PinkCrystal.ts` and `HeartOfTheTemple.ts`
  still tween the resulting `Phaser.FX.Glow | undefined` field exactly as
  before (already `if (this.glowFx) {...}`-guarded, so `undefined` is a
  normal, already-handled state) — `PinkCrystal.ts` keeps its own non-FX
  `glowBlob` Image layer as the ambient halo on mobile; the other four
  simply render without this specific glow on mobile, rather than
  broken. Desktop/tablet visuals are completely unaffected.
...
- `GameState.ts` defines `CrystalId = 'pink' | 'red' | 'green'`
  (canonical order exported as `CRYSTAL_IDS`) and a
...
- **Crystal placement (a separate, later stage of progress than
  collection):** `GameState.ts` also defines `CrystalPlacementState`
  (`getCrystalPlacementState`/`setCrystalPlaced`/`areAllCrystalsPlaced`),
  read/written only by `CrystalPlacementMode.ts` (see the "Central Hall"
  entry above) — a crystal is "collected" the moment its room's puzzle is
  solved, but "placed" only once the player has physically dragged/clicked
  it into its dedicated Central Hall slot afterward.

### Debug mode — jump straight to the final stage

Testing-only shortcut, fully isolated from normal play — a plain visit
with no `debug` query param is completely unaffected, and nothing here is
a separate/parallel state system (see `GameState.ts`'s "debug mode"
section: every effect below goes through the exact same setters a real
playthrough already calls).

**To go straight to the crystal-placement stage:** open the game with
`?debug=final` appended to the URL, e.g.:

- Local dev server: `http://localhost:5173/TempleOfNumbers/?debug=final`
- Deployed (GitHub Pages): `https://einav-gal.github.io/TempleOfNumbers/?debug=final`

This marks the intro seen, all three room-completion flags, and all three
crystals collected (`markGameIntroSeen`/`setPinkRoomState`/
`setLibraRoomState`/`setRoom3PuzzleSolved`/`setCrystalCollected` — the
same functions any real playthrough calls) — but deliberately does **not**
call `setCrystalPlaced()` for anything, so the drag/click-into-slot stage
itself stays testable rather than already finished. The game already
boots into `CentralHallScene` normally (it's the first scene in `main.ts`'s
`scene` array either way), so no scene-selection change was needed —
`CentralHallScene.create()`'s own existing registry checks do the rest.

A small, low-key on-screen tag reading `DEBUG: FINAL STAGE`
(`#debug-stage-tag` in `index.html`, toggled by `main.ts`'s
`setUpDebugStageTag()`) appears only while this is active, so it's always
obvious a session is in debug mode; it's controlled purely by
`GameState.isDebugModeActive()` and never shows on a normal visit.

**To clear debug-applied state:** `?debug=reset` (calls
`GameState.clearDebugState()`). This project has no persistence layer at
all (the registry lives only in memory for the current page load — no
`localStorage`/`sessionStorage` usage anywhere), so a normal player's
progress is never actually at risk from this — visiting `?debug=reset`
today is operationally equivalent to a fresh page load either way, but it
remains a discoverable, explicit reset affordance, and stays meaningful
without further changes if persistence is ever added later.
