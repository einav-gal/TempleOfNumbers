...
export default class HeartOfTheTemple {
  ...
  create(): void {
    ...
    // looking horizontal light streak kept recurring on devices whose
    // viewport didn't actually trigger ENVELOP, pointing to a genuine
    // mobile-GPU/WebGL Glow FX compatibility issue, not an ENVELOP-
    // viewport mismatch — see PinkCrystal.ts's matching comment). This
    // crystal has no non-FX glow layer of its own, so it simply goes
    // without a glow on mobile, rather than rendering broken.
    this.glowFx = isMobileDevice(this.scene)
      ? undefined
